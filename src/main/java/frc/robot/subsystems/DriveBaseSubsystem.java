...
public class DriveBaseSubsystem extends SubsystemBase
{
    ...
    private final TrapezoidProfile r_profile  = new TrapezoidProfile(new TrapezoidProfile.Constraints(5.0, 0.75)); // TODO: Maxrotational speed/accel?
    ...
    */
    public DriveBaseSubsystem()
    {
        ...
        xy_profile = new TrapezoidProfile(new TrapezoidProfile.Constraints( driveBaseSubsystemConfig.maximumSpeedInMeters(), 0.75)); // TODO: Max linear accel?
    ...
    */
    ...
    {
        swerveDrive.drive( xy_speed, r_speed,
                           fieldRelative, isOpenLoop, centerOfRotationMeters);
    }
    /**
    ...
    */
    private boolean SetXYSpeedsFromTarget( Translation2d current_pose )
    {
        ...
        if ( !at_xy )
        {
            ...
            velocity    = xy_setpoint.velocity; // + xy_pid.calculate( ( xy_last - xy_err ) / kDt ); test the rest first
            xy_speed    = new Translation2d( velocity * x_err / xy_err,
                                             velocity * y_err / xy_err );
        }
        else
        {
    ...
    */
    private boolean SetRSpeedFromTarget( Rotation2d r )
    {
        Double r_err = MathUtil
                .angleModulus( r_target.getRadians() - r.getRadians() );
        boolean at_r = Math.abs(r_err) < 0.1;

        if ( !at_r )
        {
            r_setpoint = r_profile.calculate( kDt, r_setpoint, r_goal );
            r_speed    = r_setpoint.velocity;  // + r_pid.calculate( ( r_last - r_err ) / kDt ); test the rest first
        }
        else
        {
    ...
    */
    private void driveFacingTarget( double x, double y )
    {
        ...
        xy_speed = new Translation2d( x, y );
        setTarget( new Rotation2d( Math.atan2( xy_target.getY() - current_pose.getY(),
                                               xy_target.getX() - current_pose.getX() ) ),
    ...
    */
    private void setTarget( Translation2d new_target, Translation2d current_pose )
    {
        ...
        xy_last     = Math.hypot( xy_target.getX() - current_pose.getX(),
                                  xy_target.getY() - current_pose.getY() );
    ...
    */
    ...
    //
    //  Command to manually translate the robot using joystick while it faces an angle
    //
    //
    ...
    */
    ...
    private class MoveFacingCommand extends Command
    {
        ...
        // Constructor
        public MoveFacingCommand ( DriveBaseSubsystem subsystem, DoubleSupplier new_x, DoubleSupplier new_y, Pose2d new_pose )
        {
            super();
            x              = new_x;
            y              = new_y;
